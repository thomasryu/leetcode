...
var totalCost = function (costs, k, candidates) {
  // Edge case where there are no leftover candidates
  ...
  // Side is either:
  // - 0 if the element comes from the left
  // - 0.1 if it comes from the right

  // We also use side as a tiebreaker, since elements
  // from the left will win over ones on the right
  const contenders = new MinPriorityQueue({
    priority: ([cost, side]) => cost + side,
  })

  for (let i = 0; i < candidates && costs.length; i++) {
    contenders.enqueue([costs.shift(), 0])
    contenders.enqueue([costs.pop(), 0.1])
  }

  for (let i = 0; i < k; i++) {
    const winner = contenders.dequeue().element
    result += winner[0]

    if (costs.length) {
      if (winner[1] == 0) contenders.enqueue([costs.shift(), 0])
      else contenders.enqueue([costs.pop(), 0.1])
    }
  }
