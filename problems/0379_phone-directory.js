...
PhoneDirectory.prototype.get = function () {
  ...
  if (this.directory.size == this.max_numbers) return -1

  if (this.next_available.length > 0) {
    next = this.next_available.shift()
    this.directory.add(next)
  } else {
    next = this.directory.size
    this.directory.add(next)
  }
...
PhoneDirectory.prototype.release = function (number) {
  if (this.directory.has(number)) {
    this.next_available.push(number)
