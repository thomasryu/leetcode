...
at a time and has to go back to the end of the line (which happens instantaneously) in order to buy more tickets.
...
var timeRequiredToBuy = function (tickets, k) {
  ...
  while (tickets.length) {
    const curr = tickets.shift() - 1
    time++

    if (k == 0 && curr == 0) return time
    if (curr) tickets.push(curr)
