...
with function ID 1 ended at the end of timestamp 2. Note that a function can be called multiple times,
...
var exclusiveTime = function (n, logs) {
  ...
  const stack = [] // 0: start_time, 1: non_exclusive_time
  const exec_times = [0] // the current execution time for each depth

  let depth = 0

  for (let log of logs) {
    const [id, op, time] = log.split(':')
    if (op == 'start') {
      stack.push(time)
      depth++
      exec_times[depth] || (exec_times[depth] = 0)
    } else {
      const start_time = stack.pop()
      const duration = time - start_time + 1

      const non_excl_time = exec_times[depth + 1] || 0
      result[id] += duration - non_excl_time
      exec_times[depth + 1] = 0

      exec_times[depth] += duration
      depth--
    }
  }
