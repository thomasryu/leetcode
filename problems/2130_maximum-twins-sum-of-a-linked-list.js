...
var pairSum = function (head) {
  ...
  let result = 0
  for (let i = 0; i < values.length - 1 - i; i++) {
    result = Math.max(result, values[i] + values[values.length - 1 - i])
  }
...
var pairSum = function (head) {
  ...
  let reversed = reverse(middle.next)
  middle.next = null

  // 3. Navigate from head and the
  //    reversed linked list to find max pair
  while (reversed && head) {
    result = Math.max(result, reversed.val + head.val)
    head = head.next
