...
var predictPartyVictory = function (senate) {
  let rSenators = senate.replaceAll('D', '').length
  let dSenators = senate.replaceAll('R', '').length

  let rBans = 0
  let dBans = 0

  const round = senate.split('')
  while (rSenators < round.length && dSenators < round.length) {
