...
You have a binary tree with a small defect. There is exactly one invalid node where
its right child incorrectly points to another node at the same depth but to the invalid node's right.
...
var correctBinaryTree = function (root) {
  ...
  while (queue.length) {
    ...
    for (let branch of ['right', 'left']) {
      if (node[branch]) {
        if (visited.has(node[branch].val)) {
          parent[side] = null
          break
        }
        new_queue.push([node[branch], node, branch])
      }
    }
