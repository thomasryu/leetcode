...
var maxAncestorDiff = function (root) {
  ...
  const dfs = (node, currMax, currMin) => {
    result = Math.max(
      result,
      Math.abs(currMax - node.val),
      Math.abs(node.val - currMin),
    )

    node.left &&
      dfs(node.left, Math.max(currMax, node.val), Math.min(currMin, node.val))
    node.right &&
      dfs(node.right, Math.max(currMax, node.val), Math.min(currMin, node.val))
  }
