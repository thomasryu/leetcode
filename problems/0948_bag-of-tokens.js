...
var bagOfTokensScore = function (tokens, power) {
  ...
  while (tokens.length) {
    ...
    // If not possible, sell most expensive
    // (only if it will possibly generate profit)
    else if (power < cheapest && score > 0 && tokens.length > 2) {
      score--
      power += tokens.pop()
    }

    // If not able to buy or sell, end
    else break
  }
