...
var lengthLongestPath = function (input) {
  input = input.split('\n')
  const is_file = (s) => s.match(/[a-zA-Z0-9]+\.[a-zA-Z0-9]+/)
  ...
  for (let f of input) {
    let curr_tier = 0
    while (f[curr_tier] == '\t') curr_tier++
    tier_length[curr_tier] = f.length - curr_tier

    if (is_file(f)) {
      let curr_length = 0
      for (let i = 0; i <= curr_tier; i++) curr_length += tier_length[i]
      result = Math.max(result, curr_length + curr_tier) // curr_tier represents the number of '/' in the path
    }
  }
