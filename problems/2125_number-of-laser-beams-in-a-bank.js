...
bank[i] represents the ith row, consisting of '0's and '1's. '0'
...
Example 1:
  ...
  Output: 8
  Explanation: Between each of the following device pairs,
    there is one beam. In total, there are 8 beams:
   * bank[0][1] -- bank[2][1]
   * bank[0][1] -- bank[2][3]
   * bank[0][2] -- bank[2][1]
   * bank[0][2] -- bank[2][3]
...
var numberOfBeams = function(bank) {
  let result = 0
  let prevCount = bank[0].replaceAll('0', '').length
    
  for (let i = 1; i < bank.length; i++) {
    const currCount = bank[i].replaceAll('0', '').length
