...
Example 2:
  ...
  Output: false
  Explanation: We can not enter room number 2 since
               the only key that unlocks it is in that room.
...
var canVisitAllRooms = function (rooms) {
  ...
  const visit = (i) => {
    ...
    if (visited == rooms.length) return true

    for (key of current) if (rooms[key] && visit(key)) return true
    return false
  }
