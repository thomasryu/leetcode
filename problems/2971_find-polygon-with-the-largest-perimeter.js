...
Example 2:
  ...
  Output: 12
  Explanation: The polygon with the largest perimeter which can be made from nums has 5 sides:
    1, 1, 2, 3, and 5. The perimeter is 1 + 1 + 2 + 3 + 5 = 12.
...
var largestPerimeter = function (nums) {
  ...
  let curr_sum = nums[0] + nums[1]
  let largest_perimeter = -1

  for (let i = 2; i < nums.length; i++) {
    if (nums[i] < curr_sum) largest_perimeter = curr_sum + nums[i]
    curr_sum += nums[i]
  }
