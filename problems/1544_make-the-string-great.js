...
var makeGood = function (s) {
  const isBad = (a, b) => {
    const badDiff = Math.abs('a'.charCodeAt(0) - 'A'.charCodeAt(0))
    return Math.abs(a.charCodeAt(0) - b.charCodeAt(0)) == badDiff
  }

  for (let i = 0; i < s.length - 1; i++) {
    const j = i + 1
    if (i < 0 || !isBad(s[i], s[j])) continue
    s = s.slice(0, i) + s.slice(j + 1, s.length)
    i -= 2
  }
