...
var leafSimilar = function (root1, root2) {
  ...
  const navigate = (node) => {
    ...
    if (node.left) navigate(node.left)
    if (node.right) navigate(node.right)
  }

  const compare = (node) => {
    if (!node) return true
    if (!node.left && !node.right) if (leaves.shift() != node.val) return false
    return compare(node.left) && compare(node.right)
  }

  navigate(root1)
  return compare(root2) && leaves.length == 0
}
