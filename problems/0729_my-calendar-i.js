...
const binarySearch = function (val, array) {
  let start = 0
  let end = array.length

  while (start < end) {
    const mid = Math.floor((start + end) / 2)
    if (val == array[mid][0]) return mid
    else if (val < array[mid][0]) end = mid
    else start = mid + 1
  }
...
MyCalendar.prototype.book = function (start, end) {
  const insert_index = binarySearch(start, this.events)

  if (insert_index > 0) {
    const prev_end = this.events[insert_index - 1][1]
    if (start < prev_end) return false
  }
  if (insert_index < this.events.length) {
    const next_start = this.events[insert_index][0]
    if (end > next_start) return false
  }

  this.events.splice(insert_index, 0, [start, end])
  return true
}
