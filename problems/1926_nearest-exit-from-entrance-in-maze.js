...
var nearestExit = function (maze, entrance) {
  ...
  while (queue.length) {
    ...
    // If we find an exit, we return the marked number of steps
    // (we also need to remember to skip the entrance itself)
    if (skippedEntrance && (x == 0 || y == 0 || x == m - 1 || y == n - 1))
      return maze[x][y]
    else skippedEntrance = true

    // For each adjacent tile
    for (step of steps) {
      const [dx, dy] = step

      // If we find a available tile, we mark it
