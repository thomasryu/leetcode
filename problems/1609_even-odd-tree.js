...
Example 1:
  ...
  Explanation: The node values on each level are:
    ...
    Level 2: [3,7,9]
    Level 3: [12,8,6,2]
    Since levels 0 and 2 are all odd and increasing and levels 1 and 3
      are all even and decreasing, the tree is Even-Odd.
...
var isEvenOddTree = function (root) {
  ...
  while (queue.length) {
    ...
    if (layer_index % 2 != node.val % 2) return false

    if (last_value) {
      if (layer_index % 2 == 0 && node.val >= last_value) return false
      if (layer_index % 2 == 1 && node.val <= last_value) return false
    }
    ...
    if (!queue.length && new_queue.length) {
      queue = new_queue
      new_queue = []
