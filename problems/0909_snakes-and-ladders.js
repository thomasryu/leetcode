...
var snakesAndLadders = function (board) {
  ...
  // Converts the square number to a
  // coordinate in the board matrix
  const positionToIndex = (square) => {
    const row = n - Math.floor((square - 1) / n) - 1
    let column = (square - 1) % n
    if ((n - row) % 2 == 0) column = n - 1 - column
    return [row, column]
  }

  const visited = new Set() // Set of visited positions
  const queue = [[1, 0]] // Array containing the [square, moves] pairs

  // Notice how, unlike previous iterations,
  // we are not using a temporary queue
  while (queue.length) {
    const [position, moves] = queue.shift()

    for (let i = 1; i <= 6; i++) {
      let nextPosition = position + i
      const [row, column] = positionToIndex(nextPosition)

      // If we find a ladder, add the ladder's position to the queue instead
      // (notice how this prevents us from using consecutive ladders since we only
      // add the ladder's destination and go directly to the following position)
      if (board[row][column] != -1) nextPosition = board[row][column]

      if (nextPosition == finish) return moves + 1
